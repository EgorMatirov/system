...
BUNSAN_STATIC_INITIALIZER(bacs_system_builders_gcc, {
  BUNSAN_FACTORY_REGISTER_TOKEN(builder, gcc,
                                [](const std::vector<std::string> &arguments) {
                                  return builder::make_shared<gcc>(arguments);
                                })
})

static const boost::regex positional("[^=]+"), key_value("([^=]+)=(.*)");

gcc::gcc(const std::vector<std::string> &arguments) : m_executable("gcc") {
  for (const std::string &arg : arguments) {
    ...
    if (boost::regex_match(arg, match, positional)) {
      ...
      if (key == "optimize") {
        ...
        if (iter == langs.end())
          BOOST_THROW_EXCEPTION(invalid_lang_error()
...
ProcessPointer gcc::create_process(const ProcessGroupPointer &process_group,
  ...
  process->setArguments(process->executable(), m_flags, name.source, "-o",
                        name.executable);
  return process;
}
