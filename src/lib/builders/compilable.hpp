...
class compilable : public builder {
 ...
 protected:
  ...
  virtual ProcessPointer create_process(
      const ProcessGroupPointer &process_group, const name_type &name) = 0;

  virtual executable_ptr create_executable(const ContainerPointer &container,
...
class compilable_executable : public executable {
 public:
  compilable_executable(const ContainerPointer &container,
                        bunsan::tempfile &&tmpdir,
